...
public class AntlineTimerRenderer implements BlockEntityRenderer<AntlineTimerBlockEntity>
{
	...
	public void renderTexture(MultiBufferSource bufferSource, PoseStack poseStack, ResourceLocation texture, int color, int alpha)
	{
		...
		color = FastColor.ARGB32.color(alpha, FastColor.ARGB32.red(color), FastColor.ARGB32.green(color), FastColor.ARGB32.blue(color));
		consumer.vertex(poseStack.last().pose(), -0.5f, -0.5f, 0).color(color).uv(0, 1).endVertex();
		consumer.vertex(poseStack.last().pose(), 0.5f, -0.5f, 0).color(color).uv(1, 1).endVertex();
		consumer.vertex(poseStack.last().pose(), 0.5f, 0.5f, 0).color(color).uv(1, 0).endVertex();
		consumer.vertex(poseStack.last().pose(), -0.5f, 0.5f, 0).color(color).uv(0, 0).endVertex();
	}
}
