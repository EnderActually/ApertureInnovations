...
{
	public static Vec3 getForward(Portal portal)
	{
		return Vec3.atLowerCornerOf(portal.getDirection().getNormal());
	}

	public static Vec3 getUp(Portal portal)
	{
		return Vec3.atLowerCornerOf(portal.getFacing().getNormal());
	}

	public static Vec3 getRight(Portal portal)
	{
		return Relative.vecRight(getForward(portal), getUp(portal));
	}

	public static Vec3 toPortalCoords(Portal portal, Vec3 vector)
	{
		Vec3 result = Relative.fromOrthogonalBasis(vector, getForward(portal),
				getUp(portal), getRight(portal));
	 ...
	 */
	public static Vec3 fromPortalCoords(Portal portal, Vec3 vector, boolean mirror)
	{
		return mirror ? Relative.toOrthogonalBasis(vector, Relative.mirrorVector(getForward(portal)), getUp(portal), Relative.mirrorVector(getRight(portal))) :
					   Relative.toOrthogonalBasis(vector, getForward(portal), getUp(portal), getRight(portal));
	}
	...
	public static class Relative
	{
		...
		public static Vec3 rotateVector(Vec3 initialVector, Direction initialDirection, Direction initialOrientation, Direction destinationDirection, Direction destinationOrientation)
		{
			Vec3 initialForward = Vec3.atLowerCornerOf(initialDirection.getNormal());
			Vec3 initialUp = new Vec3(0, 1, 0);
			if(initialOrientation.equals(Direction.UP))
				initialUp = Vec3.atLowerCornerOf(initialDirection.getOpposite().getNormal());
			if(initialOrientation.equals(Direction.DOWN))
				initialUp = Vec3.atLowerCornerOf(initialDirection.getNormal());

			Vec3 destinationForward = Vec3.atLowerCornerOf(destinationDirection.getNormal());
			Vec3 destinationUp = new Vec3(0, 1,0);
			if(destinationOrientation.equals(Direction.UP))
				initialUp = Vec3.atLowerCornerOf(destinationDirection.getOpposite().getNormal());
			if(destinationOrientation.equals(Direction.DOWN))
				initialUp = Vec3.atLowerCornerOf(destinationDirection.getNormal());
		...
		public static Vec3 preserveRelative(Direction initialDirection, Direction initialOrientation, Direction destinationDirection, Direction destinationOrientation, Vec3 initial)
		{
